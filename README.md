...
Download Manager Pro 是一款专为高并发、全自动化影视流水线打造的重型媒体集群中枢。它集成了多节点 Aria2 下载管理、云盘/盘搜直连、跨服务器集群进程监控，并搭载了行业领先的 “正则 + MP官方 + LLM大模型” 三级智能重命名引擎。
...
⚡ 本地极速提取 (Tier 1)：内置 parse-torrent-title，0 延迟、0 消耗，瞬间正则切分标准命名。

🎬 MoviePilot 官方解析 (Tier 2)：直连 MP 核心 API，精准匹配 TMDB 资料库。

🤖 LLM 大模型推理 (Tier 3)：当常规解析失效时，自动唤起大模型（OpenAI/DeepSeek/Kimi）进行深度语义提取。支持 SSE 流式传输，像 ChatGPT 一样实时打印“打字机”思考过程。
...
Bash
bash <(curl -sL https://raw.githubusercontent.com/MechanicF/Download-Manager/main/manager.sh)
...
部署完成后，在浏览器访问 http://你的IP:1111。

初始默认账号：admin，默认密码：password。

登录后，请立即前往 【⚙️ 全局设置】->【🔒 面板安全】 修改默认密码。

在设置页中依次添加你的 Aria2 节点 URL 和 RPC 密钥、云盘 API 等信息。配置实时生效！
...
📜 许可证 (License)
本项目基于 MIT License 开源。允许任何个人或企业免费使用、修改和分发，但请保留原作者版权声明。
